...
- [x] **Phase 9 — iOS token/prefs plumbing.** Networking/state layer landed in the Phase 7 commit (out of necessity — `AppDelegate` doesn't compile without it) and login/relaunch wiring in Phase 8's `handlePostSignIn()`; this phase's remaining piece — badge-count reset on becoming active (§4.9) — added here: `AppState.handleAppBecameActive()` (local `UNUserNotificationCenter.setBadgeCount(0)` + best-effort server-side `POST /api/notifications/badge/reset`), driven by a `scenePhase` observer in `IDTrackApp.swift`. Verified with a real Mac Catalyst build.
- [x] **Phase 10 — iOS Settings UI.** New "Notifications" section in `SettingsView.swift`: three toggles (New Issues / New Comments / Status Changes — the last labeled to reflect that it governs every status transition, not just →Resolved, per the resolved decision) when OS authorization is granted; a single guidance row + "Open Settings" button when denied (Apple never lets an app re-show its own permission dialog after a denial); an "Enable Notifications" button re-presenting the same soft-ask screen when `.notDetermined`. `NotificationPermissionView` was refactored to dismiss itself via `@Environment(\.dismiss)` instead of touching `appState.showNotificationPermissionPrompt` directly, so it works correctly both from MainAppView's once-per-install flow and from this Settings-local "ask again" flow (a sheet presented from a sheet). Verified with a real Mac Catalyst build.
- [x] **Phase 11 — Tap-to-open verification.** No new code, as predicted — confirmed by code inspection: `PushNotifications.swift`'s `didReceive` sets `AppState.shared?.pendingIssueSelection`; `MainAppView`'s existing `.onChange(of: appState.pendingIssueSelection, initial: true)` (built for the `idtrack://...?issue=n` share-link feature) picks it up whether MainAppView was already showing or only appears later. The `initial: true` is what makes the cold-launch-from-notification-tap case work correctly: `didReceive` can fire and set the pending value before `ContentView.boot()` has finished restoring a persisted session and before `MainAppView` exists at all — `initial: true` means MainAppView still checks the already-set value the moment it first appears, not just on subsequent changes. This is the same characteristic the idtrack:// link handling already has (see `AppState.pendingIssueSelection`'s doc comment) and is accepted as consistent, not a new edge case introduced here. Full clean-build verification: `xcodebuild ... clean build` succeeded for both Mac Catalyst and an iOS Simulator destination.
- [ ] **Phase 12 — End-to-end test pass.** See §7. Requires §5's Apple Developer prerequisites (a real `.p8` key, Key ID, Team ID configured via `idtrack default --apns-*`) and at least one physical device — not runnable from this environment. One piece of §5 is already confirmed done: a real `xcodebuild` build with this machine's actual signing identity produced a provisioning profile carrying `com.apple.developer.aps-environment`, so the App ID already has the Push Notifications capability enabled on the Apple Developer portal side.
