...
**Attachments support PDF and text in addition to images, sniffed server-side rather than trusted from the client, and PDF thumbnails are a generic icon rather than a rendered first page — a deliberate scope decision, not a missing feature.** `attachments.type` (`"image"` | `"pdf"` | `"text"`) was added via the usual `addColumnIfMissing` + guarded `WHERE type = ''` backfill (every pre-existing attachment was, by definition, an image before this feature existed). `server/images.go`'s `detectAttachmentKind` sniffs the kind by content alone — `%PDF-` magic bytes, else a successful `image.Decode`, else a conservative binary-vs-text heuristic (valid UTF-8, no NUL bytes) — the same "never trust a client-declared Content-Type or filename" rule the original image-only version of this feature already followed for HEIC rejection. `image` still converts to PNG and gets a real scaled thumbnail exactly as before; `text` gets a real thumbnail too (an image of the file's first 45 lines, rendered with `golang.org/x/image/font/basicfont` — a font already available transitively via the existing `golang.org/x/image` dependency, so this added no new module). `pdf` was evaluated for true first-page rasterization and rejected: every pure-Go, CGO-free rendering path either doesn't do real page rasterization at all (`pdfcpu`, `unidoc/unipdf` free tier — text/metadata extraction only) or pulls in a genuinely heavy new dependency (`gen2brain/go-fitz` requires cgo + a system MuPDF library, breaking `CGO_ENABLED=0` static builds the same way HEIC support would; `klippa-app/go-pdfium` stays CGO-free only by bundling a PDFium build compiled to WebAssembly and run through a pure-Go WASM runtime, adding tens of MB and real runtime overhead). Given that tradeoff, PDF instead gets `genericThumbnail` — a synthesized document-icon-with-folded-corner plus the filename captioned underneath, drawn with the same `basicfont` machinery as the text thumbnail — which doubles as the safety-net thumbnail for any future attachment kind added without its own thumbnail logic. The thumbnail endpoint (`GET /api/attachments/{aid}/thumbnail`) always returns `image/png` regardless of kind; the full-blob endpoint's `Content-Type` varies by `type` (`image/png` / `application/pdf` / `text/plain; charset=utf-8`), computed by `attachmentContentType` in `server/attachments.go`. The multipart upload field was renamed from `image` to `file` (`attachmentFormField`) to match, since it's no longer image-only — a deliberate breaking change to the upload API, acceptable pre-merge on the feature branch this shipped on.

**The web attachment viewer doesn't attempt an inline PDF preview at all — a plain "can't preview this" message plus the Save button is what a kind with no renderer gets, deliberately, after inline embedding was tried and dropped.** The first version pointed a PDF at an `<iframe>`/`<embed>` via a `blob:` URL built from a JS-fetched copy (working around the server's own `frame-ancestors 'none'` CSP, which blocks framing its own responses even from this same page — `frame-ancestors` governs who may frame a *response* regardless of who's doing the framing). That rendered blank in both a Playwright-driven headless Chromium (no PDF-viewer plugin registered for `<embed>`; switching to `<iframe>` didn't help either — same blank result) and in real-browser dogfooding, so the underlying premise — that a same-page inline PDF preview is dependable — didn't hold. `openAttachmentViewer` (`resources/idtrack.js`) now branches to `attachmentUnavailableMessage()` for any type without its own preview (currently just `pdf`; the branch is the fallback for any future kind too), showing a fixed notice (`#av-unavailable`) that points the user at Save rather than risking a preview that might silently render nothing — a message the user can trust beats a maybe-blank embed. Text is unaffected by any of this: it's fetched via `apiGetBlob` (an authenticated `fetch` returning a `Blob`) and shown as plain text in a `<pre>`, which is a client-side fetch, not a framed response, so `frame-ancestors` never applied to it. Plain `<img>` (the image-attachment path) was never affected either, since `frame-ancestors` only governs framing elements (`iframe`/`embed`/`object`), not ordinary resource loads — so no CSP change was needed once the iframe attempt was removed.
