...
│   └── pdf.go            # openPDFDocument/pdfPageCount/renderPDFPage — github.com/tucats/pdf-viewer wrappers
...
CREATE TABLE attachments (
    ...
    type        TEXT NOT NULL DEFAULT 'image', -- "image" | "pdf" | "text" — see db.AttachmentType
    width       INTEGER NOT NULL DEFAULT 0, -- of the stored (converted) PNG; 0 for pdf/text
    height      INTEGER NOT NULL DEFAULT 0,
    size        INTEGER NOT NULL DEFAULT 0, -- original byte length of the file, independent of whether image is stored gzip-compressed
    pages       INTEGER NOT NULL DEFAULT 0, -- PDF page count; 0 for non-PDF, or an uncomputed legacy PDF — see db.SetAttachmentPages
    compressed  INTEGER NOT NULL DEFAULT 0, -- 1 if the image column holds gzip-compressed bytes; see db.CreateAttachment/db.compressBlob
    image       BLOB NOT NULL,             -- full-size PNG (image), or the raw original bytes (pdf/text) — gzip-compressed on disk when compressed=1
...
| GET | `/api/attachments/{aid}/page/{n}` | yes | no |
...
| GET | `/api/notifications/stream` | yes | no (self-service only) |
...
_notifyStream     // EventSource | null — open GET /api/notifications/stream connection; see NOTIFICATIONS.md §9
_desktopNotify    // bool — client-side "Desktop notifications" preference; mirrors idtrack_prefs, default false (opt-in)
```
...
**Attachments support PDF and text in addition to images, sniffed server-side rather than trusted from the client.** `attachments.type` (`"image"` | `"pdf"` | `"text"`) was added via the usual `addColumnIfMissing` + guarded `WHERE type = ''` backfill (every pre-existing attachment was, by definition, an image before this feature existed). `server/images.go`'s `detectAttachmentKind` sniffs the kind by content alone — `%PDF-` magic bytes, else a successful `image.Decode`, else a conservative binary-vs-text heuristic (valid UTF-8, no NUL bytes) — the same "never trust a client-declared Content-Type or filename" rule the original image-only version of this feature already followed for HEIC rejection. `image` converts to PNG and gets a real scaled thumbnail; `text` gets a real thumbnail too (an image of the file's first 45 lines, rendered with `golang.org/x/image/font/basicfont`); `pdf` gets a real render of its first page — see the `github.com/tucats/pdf-viewer` entry below for how, and for the earlier, now-superseded decision to reject true rasterization. The thumbnail endpoint (`GET /api/attachments/{aid}/thumbnail`) always returns `image/png` regardless of kind; the full-blob endpoint's `Content-Type` varies by `type` (`image/png` / `application/pdf` / `text/plain; charset=utf-8`), computed by `attachmentContentType` in `server/attachments.go`. The multipart upload field was renamed from `image` to `file` (`attachmentFormField`) to match, since it's no longer image-only — a deliberate breaking change to the upload API, acceptable pre-merge on the feature branch this shipped on.

**PDF pages are rendered server-side via `github.com/tucats/pdf-viewer`, a pure-Go, CGO-free PDF renderer — reversing an earlier decision to reject true rasterization.** When PDF attachment support first shipped, every CGO-free rendering path available at the time either didn't do real page rasterization at all (`pdfcpu`, `unidoc/unipdf` free tier — text/metadata extraction only) or pulled in a genuinely heavy dependency (`gen2brain/go-fitz` requires cgo + a system MuPDF library, breaking `CGO_ENABLED=0` static builds the same way HEIC support would; `klippa-app/go-pdfium` stays CGO-free only by bundling a WASM-compiled PDFium run through a pure-Go WASM runtime, adding tens of MB and real runtime overhead) — so PDFs got a generic icon+filename thumbnail (`genericThumbnail`, `server/images.go`) instead, and the web viewer showed a plain "can't preview this" message for PDF specifically (see the next entry). `github.com/tucats/pdf-viewer` closes that gap: it renders a page to an `image.Image` (`Page.Render`/`Page.Thumbnail`) with no CGO, no `os/exec`, and no network access, satisfying the same build-portability constraint every other dependency choice in this doc follows. `server/pdf.go`'s `openPDFDocument` opens every document via `pdfviewer.WithFontSubstitution(pdfviewer.FontSubstitution{})` — the zero-value `FontSubstitution` deliberately passed explicitly rather than omitting the option entirely, since omitting it disables substitution altogether (`WithFontSubstitution` is what turns the feature on at all) while the zero value's `DisableSystemDefaults: false` also scans the platform's default font directories — so a PDF referencing a font it doesn't embed a program for gets a real substituted outline instead of pdf-viewer's built-in notdefGlyph placeholder box, which real-world PDFs need often enough that skipping this would make many ordinary documents look badly broken. `renderPDFPage`/`pdfPageCount` (`server/pdf.go`) are pdf-viewer's only two call sites, both wrapped with a `pdfRenderTimeout` (10s) via `context.WithTimeout` as defense-in-depth against a pathological content stream — pdf-viewer's own `maxRenderPixels` cap already bounds output size, but not necessarily render time — matching this file's existing posture toward untrusted attachment content (`maxImagePixels`, the HEIC 415, `detectAttachmentKind` never trusting client-declared type). `processUploadedPDF` (`server/images.go`) computes the page count and renders the first-page thumbnail at upload time, independently falling back to `pages = 0` and/or `genericThumbnail` on either failure — a single malformed or unusual PDF still uploads successfully, matching this file's general "never let one bad attachment break the request" rule.

A PDF's page count is cached on the attachment row (`attachments.pages`, added via the usual `addColumnIfMissing`) rather than reparsed on every access. Unlike every other backfilled column in this file, a PDF uploaded before this feature existed is **not** backfilled eagerly at startup (`db.SetAttachmentPages`'s doc comment) — computing a page count means opening and parsing every stored PDF, which could be slow or even fail for a database with many/large attachments, so `pages` starts (and stays) `0` for a legacy row until `GET /api/attachments/{aid}/page/{n}` (`handleGetAttachmentPage`, `server/attachments.go`) computes and caches it lazily on first request. That same endpoint is 1-based in the URL (matching the human-facing "N / total" counter) even though pdf-viewer itself indexes pages zero-based; it returns the total via an `X-Attachment-Pages` response header so the client can learn (or correct) the count from any single page fetch without a separate metadata round-trip.

**The web attachment viewer's PDF page images are loaded via a plain `<img src>` pointed at the page endpoint, not a `blob:` object URL — the latter was tried first and silently failed under the server's own CSP.** The natural approach for reading `X-Attachment-Pages` off the response would be an authenticated `fetch` returning a `Blob`, turned into an `<img>` source via `URL.createObjectURL` — exactly the pattern `saveAttachment` already uses for its plain-download fallback. That works for the fetch/header-reading itself, but the resulting `blob:` URL fails Chromium's own CSP enforcement of this server's `img-src 'self' data:` directive (verified with a Playwright-driven headless-browser test during development — the browser silently drops the image with a CSP console error, no exception the code could catch), and widening `img-src` to allow `blob:` was rejected as unnecessarily loosening a deliberately strict header for what turns out to have an easy alternative. `avPdfLoadPage` (`resources/idtrack.js`) instead makes its authenticated `fetch` purely to check the response status and read the header, then points `#av-image`'s `src` directly at the same same-origin URL — the browser reuses that fetch's own response rather than requesting it twice, since every attachment route already sets `Cache-Control: ...immutable`. This is the same direct-`<img src>` pattern the image-attachment path (and, before this feature, nothing PDF-related) already used; PDF just needed the extra fetch step first to get at the header and to distinguish a "no such page" 404 (e.g. `Next` clicked past a legacy PDF's not-yet-known last page — clamped silently rather than shown as an error) from a genuine render failure.

**The web attachment viewer no longer treats PDF as a kind with no inline preview — `attachmentUnavailableMessage()` remains only as the fallback for some future kind added without one.** Before server-side rendering existed, the viewer's first version pointed a PDF at an `<iframe>`/`<embed>` via a `blob:` URL built from a JS-fetched copy (working around the server's own `frame-ancestors 'none'` CSP, which blocks framing its own responses even from this same page — `frame-ancestors` governs who may frame a *response* regardless of who's doing the framing). That rendered blank in both a Playwright-driven headless Chromium (no PDF-viewer plugin registered for `<embed>`; switching to `<iframe>` didn't help either — same blank result) and in real-browser dogfooding, so PDF fell back to the same plain "can't preview this" notice (`#av-unavailable`) any other unhandled kind gets. Text is unaffected by any of this history: it's fetched via `apiGetBlob` and shown as plain text in a `<pre>`, a client-side fetch rather than a framed response, so `frame-ancestors` never applied to it; plain `<img>` (the image-attachment path) was never affected either, since `frame-ancestors` only governs framing elements, not ordinary resource loads.

**Attachment blobs are transparently gzip-compressed at rest when it's actually worth it, entirely inside the `db` package — no caller above it, including every HTTP handler, ever needs to know.** As the attachments table grew, the main blob (the `image` column — the actual file content, not the always-small thumbnail) became the obvious place to reclaim space. `db.compressBlob` (`db/attachments.go`) only even attempts gzip on a blob of at least `compressMinSize` (1024 bytes) — trying on anything smaller has no realistic chance of clearing the next threshold, given gzip's own ~18-20 byte header/footer overhead — and only *keeps* the compressed form if it saves at least `compressMinSavings` (1024 bytes) versus the original; otherwise the original bytes are stored as-is. That savings check matters in practice, not just in theory: image attachments are already-compressed PNG, and many PDFs already Flate-compress their own internal streams, so a second gzip pass over the whole file frequently saves little or nothing — without the check, every read of such a file would pay a pointless decompression cost forever. `db.CreateAttachment` calls `compressBlob` on `file` before the `INSERT` and records the outcome in the new `compressed` column (`addColumnIfMissing`, `DEFAULT 0`); `db.GetAttachmentImage` checks that column and gunzips via `db.decompressBlob` before returning, so every caller — `handleGetAttachmentImage`, `renderPDFPage`/`pdfPageCount` (`server/pdf.go`), the thumbnail-generation code that reads a just-uploaded PDF back — sees exactly the original bytes it always did, unaware compression exists at all. `DEFAULT 0` is not merely a safe placeholder the way some other `DEFAULT 0` columns in this schema are: it is the *only correct* value for every attachment that predates this feature, since none of them were ever compressed, so — unlike `resolved_at`'s or the teams columns' backfills — there is nothing to backfill and no reason to ever try (retroactively compressing existing rows would mean reading and rewriting every one of them for a purely internal storage optimization no caller can observe either way). The `size` column and the `Attachment.Size` JSON field are deliberately left untouched by any of this — both are always `len(file)`, the original byte count, since that's the meaningful "how big is this file" answer an API consumer actually wants, not an internal, potentially smaller on-disk number that has nothing to do with the file's own real characteristics. Only the main blob is ever compressed; the thumbnail column is not, since it's already small (typically well under `compressMinSize`) and already PNG-encoded, so a second compression pass over it would rarely help and was judged not worth a second `compressed` flag for the marginal cases where it might.

**Every attachment can be saved to the user's local filesystem from the viewer, via the File System Access API where available and a classic `<a download>` otherwise — the same graceful-degradation shape the PDF "can't preview" case leans on.** `saveAttachment(id, filename)` (`resources/idtrack.js`) calls `window.showSaveFilePicker` first, before any `await`, specifically so it still runs inside the click handler's user-activation window — calling it after an intervening `await` risks a browser treating it as not user-initiated and rejecting it. When that API isn't available (Firefox, Safari as of this writing), or the user cancels the picker (`AbortError`, handled as a silent no-op, not a failure), it falls back to building a `blob:` object URL and clicking a detached `<a download>` — the standard trigger-a-save-without-navigating trick, which every browser turns into a normal downloads-folder save. This is the same `apiGetBlob` fetch the text preview already used, so no new fetch machinery was needed — Save just works for every kind uniformly, including the ones (PDF) that have no inline preview at all.

**The PDF generic-icon thumbnail uses a scaled-up, high-contrast colored badge for its label rather than the bitmap font at native size, after the first version proved too low-contrast to read at a glance.** `basicfont.Face7x13`'s native 7×13 glyphs are legible at the thumbnail's full 320px canvas but wash out once the browser scales the thumbnail down to the CSS grid's actual 72×72 display size. `drawScaledLabel` (`server/images.go`) renders the label onto a small offscreen bitmap at native size, then nearest-neighbor scales it up (`genericBadgeScale`, currently 4×) — nearest-neighbor rather than a smooth interpolator deliberately, since smoothing a 1-bit-per-pixel bitmap font just blurs it rather than making it look "bigger." `drawBadgeLabel` first paints a solid, strongly-colored rectangle (`genericBadgeBg`, a saturated red — the familiar "PDF badge" look many file-manager icons use) behind the scaled label for contrast, sized to fit the label plus fixed padding and centered on the document icon's page rect. The same `drawScaledLabel` helper is reusable for any future generic-icon kind that wants a bold label the same way.
...
**Web in-app notifications reuse the exact same trigger rules and preferences as push, delivered over Server-Sent Events instead of APNs.** Full design rationale: [docs/NOTIFICATIONS.md §9](docs/NOTIFICATIONS.md#9-web-in-app-notifications). `server/webnotify.go`'s `webNotifyHub` is an in-memory `map[username][]chan webNotifyEvent` (mirroring `sessionStore`'s shape, with potentially many subscriber channels per user — one per open browser tab, the same one-user-many-endpoints shape `TokensForUser` has for APNs tokens), constructed unconditionally in `server.Start` — unlike `s.apns`/`s.webauthn`, in-app notification needs no operator opt-in, so `notify.go`'s `notifyOne` fans out to it even when `s.apns == nil` (APNs not configured). `GET /api/notifications/stream` is a long-lived SSE connection, deliberately excluded from two pieces of existing middleware that assume a request finishes shortly after it starts: `quiesce` (`server/backup.go`) would otherwise hold its per-request `backupMu.RLock()` for a connection's entire lifetime, permanently starving `doBackup`'s writer lock the moment any browser tab is open; `gzipHandler` (`server/compress.go`) buffers an entire response body until the handler returns, which for a stream is never, until the client disconnects. Both bypasses key off `isNotificationStreamRequest` (`server/middleware.go`), matched by path suffix so `--base-path` prefixing doesn't break the check. The handler also disables `http.Server`'s `WriteTimeout`/`ReadTimeout` per-connection via `http.NewResponseController` (they would otherwise silently kill every stream after `WriteTimeout` elapses) and sends `X-Accel-Buffering: no` so a reverse-proxy deployment doesn't buffer the stream at that layer instead. Client-side (`resources/idtrack.js`), `startNotifyStream()`/`stopNotifyStream()` open/close a plain `EventSource` from the same choke points `startPolling()`/`stopPolling()` already use (`launchApp()`, `doLogout()`, and `apiFetch`'s 401 handler); every event renders as a dismissible, auto-expiring toast (`showNotifyToast`, styled distinctly from the pre-existing `#refresh-hint` toast) and, only when the tab is backgrounded and the user has opted in via Settings' "Desktop notifications" toggle (`toggleDesktopNotify`, gated on the browser's `Notification` permission), also raises an OS-level desktop notification. The existing 30-second `pollForChanges` loop is unchanged and keeps running — SSE is additive, degrading gracefully to the pre-existing poll-driven behavior if a stream never connects.

**The three `notify_*` preferences are account-wide and editable from web Settings, not just the iOS app's.** Before this, the web client only ever *read* these preferences (to decide whether to show a toast); Settings now has a "Notifications" section (`loadNotificationPrefs`/`saveNotificationPrefs` in `resources/idtrack.js`) wired directly to the existing, already-self-service `GET`/`PUT /api/notifications/prefs` endpoints — no new API surface. `loadNotificationPrefs` always re-fetches when Settings opens rather than trusting a cached value, since another client (the iOS app, another browser) may have changed them since. A `.hint` caption makes explicit that these three toggles are account-wide — changing one here changes push on every device too — deliberately distinguished from the adjacent, separate "Desktop notifications" toggle (browser-local: whether *this* browser pops up an OS notification, never which categories are received).
