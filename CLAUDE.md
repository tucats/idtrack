...
- **Any → Blocked** (`#blocked-overlay`): required dialog capturing one or more blocking issue IDs (chip list with add/remove) plus an optional extra comment appended to the server-generated *"Blocked by issues #N, #M…"* note. The optional text is sent as `comment` in the PUT body — not as a separate POST. After confirming, the server stores the IDs in `dependent_issues`.
...
**Server-side validation for Blocked/Duplicate transitions.** `handleUpdateIssue` in `server/issues.go` enforces: Duplicate → exactly one `dependent_issues` ID that exists and is not self; Blocked → at least one ID, all existing, none self; non-admins cannot *remove* IDs from an already-Blocked issue's list (they can only add); Blocked→Open requires every dependent issue to have `status = 'Resolved'` (HTTP 409 otherwise). On transition TO Duplicate or Blocked the server auto-posts a system comment via `db.CreateComment` — errors from that call are ignored so the status change is never rolled back by a comment failure. For Open/Resolved the handler clears `dependent_issues` automatically regardless of what the client sends.
