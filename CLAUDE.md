...
**The web attachment viewer fetches PDF/text bytes via authenticated JS and renders them via `blob:` URL / `<pre>`, rather than pointing an `<iframe>`/`<img>` straight at the attachment API URL — because the server's own `frame-ancestors 'none'` CSP blocks framing its own responses, even from this same page.** `frame-ancestors` governs who may frame a *response*, regardless of who's doing the framing, so a same-origin `<iframe src="/api/attachments/{id}">` is blocked exactly like a third-party site attempting the same thing would be. `openAttachmentViewer` (`resources/idtrack.js`) instead calls `apiGetBlob` (an authenticated `fetch` returning a `Blob`, alongside `apiGet`/`apiPost`/etc.) and either creates an object URL via `URL.createObjectURL` for a PDF's `<iframe>` (revoked on close/reselect via `revokeAvObjectUrl` — an un-revoked `blob:` URL leaks the fetched bytes for the page's lifetime) or reads the blob as text for a `<pre>` block — neither of which is a second framed HTTP response, so `frame-ancestors` doesn't apply. The PDF viewer is an `<iframe>`, not an `<embed type="application/pdf">`: `<embed>` depends on the browser having a legacy NPAPI-style PDF plugin registered, which is inconsistent across Chromium builds (confirmed by hand during development — Playwright's bundled headless Chromium has no such plugin and showed "Couldn't load plugin"), whereas `<iframe>` is what MDN recommends and what every evergreen browser renders a PDF response inside using its own built-in viewer. This also means the CSP itself needed a new directive: `frame-src blob:` (added to `secureHeaders` in `server/middleware.go`) — without it, `frame-src` falls back to `default-src 'self'`, which doesn't cover `blob:`, and the browser silently refuses to render the iframe's content. Plain `<img>` (the pre-existing image-attachment path) is unaffected by any of this, since `frame-ancestors`/`frame-src` only govern framing elements (`iframe`/`embed`/`object`), not ordinary resource loads.
